...
// #include "OCVUtils.h" // temporary not including because duplicate symbol
// err. (about Points)
...
public:
  ...
  bool resetTracking(cv::Mat frame, size_t cols, size_t rows) {
    ...
    for (int i = 0; i < _trackables.size(); i++) {
      _trackables[i]._orb->detectAndCompute(frame, cv::noArray(), frameKeyPts,
                                            frameDescr);
    ...
    for (size_t i = 0; i < knnMatches.size(); ++i) {
      if (knnMatches[i][0].distance <
          nn_match_ratio * knnMatches[i][1].distance) {
  ...
  bool track(cv::Mat frame, size_t frameCols, size_t frameRows) {
    ...
    if ((goodPtsCurr.size() > numMatches / 2) && (1.75 > avg_variance)) {
      ...
      if ((valid = homographyValid(_H))) {
        ...
        cv::perspectiveTransform(_trackables[0]._bBox, warped, _H);
        EM_ASM_({
          self.postMessage({ type: 'warped', data: [$0, $1, $2, $3, $4, $5, $6, $7] });
        }, warped[0].x, warped[0].y, warped[1].x, warped[1].y, warped[2].x,
           warped[2].y, warped[3].x, warped[3].y);
    ...
    for (int i = 0; i < _trackables.size(); i++) {
      if ((_trackables[i]._isDetected) || (_trackables[i]._isTracking)) {
        ...
        // _trackables[i]._trackSelection.GetSelectedFeatures3d(); ARLOGi("start
        // pose matrix\n"); CameraPoseFromPoints(_trackables[i]._pose,
        // objPoints, imgPoints);
...
// Convert it to Gray
  ...
  void AddMarker(unsigned char *buff, std::string fileName, int width,
                 int height, int uid, float scale) {
    ...
    if (!newTrackable._image.empty()) {
      ...
      // newTrackable._featurePoints =
      // _featureDetector.DetectAndCompute(newTrackable._image, cv::Mat(),
      // newTrackable._descriptors);
      std::cout << "Add Marker CalcDescriptors" << std::endl;
      // newTrackable._descriptors =
      // _featureDetector.CalcDescriptors(newTrackable._image,
      // newTrackable._featurePoints);
      std::cout << "Add Marker FindCorners" << std::endl;
      newTrackable._cornerPoints =
          _harrisDetector.FindCorners(newTrackable._image);
      newTrackable._orb = cv::ORB::create(MAX_FEATURES);
      newTrackable._orb->detectAndCompute(newTrackable._image, cv::noArray(),
                                          refKeyPts, refDescr);
      ...
      newTrackable._bBox.push_back(
          cv::Point2f(newTrackable._width, newTrackable._height));
      ...
      newTrackable._trackSelection =
          TrackingPointSelector(newTrackable._cornerPoints, newTrackable._width,
  ...
  bool IsImageInitialized() { return initialized; }
  ...
  void CameraPoseFromPoints(cv::Mat &pose, std::vector<cv::Point3f> oPts,
                            std::vector<cv::Point2f> iPts) {
    cv::Mat rvec = cv::Mat::zeros(3, 1, CV_64FC1); // output rotation vector
    cv::Mat tvec = cv::Mat::zeros(
        3, 1, CV_64FC1); // output translation vector
// std::cout << "Inside cameraPose" << std::endl;
//  --llvm-lto 1 compiler setting breaks the solvePnPRansac function on iOS but
//  using the solvePnP function is faster anyways
#if ARX_TARGET_PLATFORM_EMSCRIPTEN
...
#else
    bool solvePnP =
        cv::solvePnPRansac(oPts, iPts, _K, cv::noArray(), rvec, tvec);
  ...
  void CameraPoseD(cv::Mat &pose, cv::Mat Homography) {
    ...
    int solutions = decomposeHomographyMat(Homography, _K, Rs_decomp, ts_decomp,
                                           normals_decomp);
  ...
  TrackedOrbImageInfo GetTrackableImageInfo(int trackableId) {
    ...
    for (int i = 0; i < _trackables.size(); i++) {
      if (_trackables[i]._id == trackableId) {
        ...
        unsigned char *data = (unsigned char *)malloc(_trackables[i]._width *
                                                      _trackables[i]._height);
        memcpy(data, _trackables[i]._image.ptr(),
               _trackables[i]._width * _trackables[i]._height);
...
PlanarOrbTracker::PlanarOrbTracker()
    : _trackerImpl(new PlanarOrbTrackerImpl()) {}
...
void PlanarOrbTracker::Initialise(int xFrameSize, int yFrameSize,
                                  ARdouble cParam[][4]) {
...
void PlanarOrbTracker::RemoveAllMarkers() { _trackerImpl->RemoveAllMarkers(); }

void PlanarOrbTracker::AddMarker(unsigned char *buff, std::string fileName,
                                 int width, int height, int uid, float scale) {
