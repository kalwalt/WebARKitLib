...
WebARKitVideoView::WebARKitVideoView() :
...
bool WebARKitVideoView::initWithVideoSource(const WebARKitVideoSource& vs, const int contextWidth, const int contextHeight)
{
    ARLOGd("WebARKitVideoView::initWithVideoSource.\n");
...
void WebARKitVideoView::update()
{
    ...
    m_viewport[2] = w;
    m_viewport[3] = h;
    ARLOGd("WebARKitVideoView::update() calculated viewport {%d, %d, %d, %d}\n", left, bottom, w, h);
}

bool WebARKitVideoView::distortionCompensation() const
{
    return m_distortionCompensation;
}

void WebARKitVideoView::setDistortionCompensation(const bool distortionCompensation)
{
...
bool WebARKitVideoView::rotate90() const
{
    return m_rotate90;
}

void WebARKitVideoView::setRotate90(const bool rotate90)
{
...
bool WebARKitVideoView::flipH() const
{
    return m_flipH;
}

void WebARKitVideoView::setFlipH(const bool flipH)
{
    if (m_flipH != flipH) {
        m_flipH = flipH;
        update();
...
bool WebARKitVideoView::flipV() const
{
    return m_flipV;
}

void WebARKitVideoView::setFlipV(const bool flipV)
{
    if (m_flipV != flipV) {
        m_flipV = flipV;
        update();
...
WebARKitVideoView::HorizontalAlignment WebARKitVideoView::horizontalAlignment() const
{
    return m_hAlign;
}

void WebARKitVideoView::setHorizontalAlignment(const HorizontalAlignment hAlign)
{
    if (m_hAlign != hAlign) {
        m_hAlign = hAlign;
        update();
...
WebARKitVideoView::VerticalAlignment WebARKitVideoView::verticalAlignment() const
{
    return m_vAlign;
}

void WebARKitVideoView::setVerticalAlignment(const VerticalAlignment vAlign)
{
    if (m_vAlign != vAlign) {
        m_vAlign = vAlign;
        update();
...
WebARKitVideoView::ScalingMode WebARKitVideoView::scalingMode() const
{
    return m_scalingMode;
}

void WebARKitVideoView::setScalingMode(const ScalingMode scaling)
{
...
WebARKitVideoView::Size WebARKitVideoView::contextSize() const
{
    return {m_contextWidth, m_contextHeight};
}

void WebARKitVideoView::setContextSize(const Size size)
{
...
void WebARKitVideoView::getViewport(int32_t viewport[4]) const
{
    if (!viewport) return;
...
void WebARKitVideoView::draw(WebARKitVideoSource* vs)
{
    // If glStateCache is in use, need to flush it here, since we can't guarantee external users will have been using it.
#if USE_GL_STATE_CACHE
    glStateCacheFlush();
    ...
    if (frame) {
        ...
        ARLOGd("WebARKitVideoView::draw frame=NULL.\n");
    }
    arglDispImage(m_arglContextSettings, m_viewport);
}

void WebARKitVideoView::drawDebugImage(const uint8_t *image, int pyramidLevel)
{
    // If glStateCache is in use, need to flush it here, since we can't guarantee external users will have been using it.
#if USE_GL_STATE_CACHE
    glStateCacheFlush();
    ...
    if (image) {
        ...
        ARLOGd("WebARKitVideoView::drawDebugImage image=NULL.\n");
    }
    arglDispImage(m_arglContextSettings, m_viewport);
}
