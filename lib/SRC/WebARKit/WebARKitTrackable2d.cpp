...
WebARKitTrackable2d::~WebARKitTrackable2d()
{
    if (m_loaded) unload();
}

bool WebARKitTrackable2d::load(const char* dataSetPathname_in)
{
    // Load data.
...
bool WebARKitTrackable2d::load2DData(const char* dataSetPathname_in, std::shared_ptr<unsigned char> refImage, int refImageX, int refImageY)
{
    if (m_loaded) unload();
...
bool WebARKitTrackable2d::updateWithTwoDResults(int detectedPage, float trackingTrans[3][4], ARdouble transL2R[3][4])
{
...
float WebARKitTrackable2d::TwoDScale()
{
    return (m_twoDScale);
}
