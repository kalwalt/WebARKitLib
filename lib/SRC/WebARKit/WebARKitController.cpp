...
WebARKitController::WebARKitController() :
...
const char* WebARKitController::getARToolKitVersion()
{
    if (!versionString) arGetVersion(&versionString);
	return versionString;
}

void WebARKitController::setError(int error)
{
...
int WebARKitController::getError()
{
    int temp = m_error;
...
bool WebARKitController::initialiseBase()
{
    ARLOGd("WebARKit::WebARKitController::initialiseBase(...)\n");
    ...
    m_squareTracker = std::shared_ptr<WebARKitTrackerSquare>(new WebARKitTrackerSquare);
    if (!m_squareTracker->initialize()) {
        ARLOGe("Error initialising square marker tracker.\n");
        goto bail;
    }
#if HAVE_NFT
    m_nftTracker = std::shared_ptr<WebARKitTrackerNFT>(new WebARKitTrackerNFT);
...
bool WebARKitController::startRunning(const char* vconf, const char* cparaName, const char* cparaBuff, const long cparaBuffLen)
{
	ARLOGi("Starting...\n");
	...
	m_videoSource0 = new WebARKitVideoSource;
	if (!m_videoSource0) {
    ...
    if (!m_videoSource0->open()) {
        if (m_videoSource0->getError() == WEBARKIT_ERROR_DEVICE_UNAVAILABLE) {
    ...
    ARLOGd("WebARKitController::startRunning(): done.\n");
	return true;
}

bool WebARKitController::startRunningStereo(const char* vconfL, const char* cparaNameL, const char* cparaBuffL, const long cparaBuffLenL,
                                      ...
                                      const char* transL2RName, const char* transL2RBuff, const long transL2RBuffLen)
{
	...
	m_videoSource0 = new WebARKitVideoSource;
	m_videoSource1 = new WebARKitVideoSource;
    ...
    if (!m_videoSource1->open()) {
        if (m_videoSource1->getError() == WEBARKIT_ERROR_DEVICE_UNAVAILABLE) {
    ...
    ARLOGd("WebARKitController::startRunningStereo(): done.\n");
	return true;
}

bool WebARKitController::capture()
{
...
bool WebARKitController::updateTextureRGBA32(const int videoSourceIndex, uint32_t *buffer)
{
...
bool WebARKitController::stopRunning()
{
    ...
    ARLOGd("WebARKit::WebARKitController::stopRunning(): done.\n");
	return true;
}

bool WebARKitController::shutdown()
{
    ARLOGd("WebARKit::WebARKitController::shutdown()\n");
    do {
        switch (state) {
            case DETECTION_RUNNING:
            case WAITING_FOR_VIDEO:
                ARLOGd("WebARKit::WebARKitController::shutdown(): DETECTION_RUNNING or WAITING_FOR_VIDEO, forcing stop.\n");
                stopRunning();
            break;

            case BASE_INITIALISED:
                if (countTrackables() > 0) {
                    ARLOGd("WebARKit::WebARKitController::shutdown(): BASE_INITIALISED, cleaning up trackables.\n");
                    removeAllTrackables();
                }
            ...
            case NOTHING_INITIALISED:
                ARLOGd("WebARKit::WebARKitController::shutdown(): NOTHING_INITIALISED, complete\n");
            break;
        }
    } while (state != NOTHING_INITIALISED);

    ARLOGi("webarkit finished.\n");
	return true;
}
...
bool WebARKitController::drawVideo(const int videoSourceIndex)
{
    if (videoSourceIndex < 0 || videoSourceIndex > (m_videoSourceIsStereo ? 1 : 0)) return false;
...
bool WebARKitController::projectionMatrix(const int videoSourceIndex, const ARdouble projectionNearPlane, const ARdouble projectionFarPlane, ARdouble proj[16])
{
    if (videoSourceIndex < 0 || videoSourceIndex > (m_videoSourceIsStereo ? 1 : 0)) return false;
...
bool WebARKitController::isInited()
{
	// Check we are in a valid state to add a trackable (i.e. base initialisation has occurred)
	return (state != NOTHING_INITIALISED);
}

bool WebARKitController::isRunning()
{
	return state == DETECTION_RUNNING;
}

bool WebARKitController::videoParameters(const int videoSourceIndex, int *width, int *height, AR_PIXEL_FORMAT *pixelFormat)
{
    if (videoSourceIndex < 0 || videoSourceIndex > (m_videoSourceIsStereo ? 1 : 0)) return false;
...
int WebARKitController::addTrackable(const std::string& cfgs)
{
	if (!isInited()) {
...
// private
bool WebARKitController::addTrackable(WebARKitTrackable* trackable)
{
    ARLOGd("WebARKitController::addTrackable(): called\n");
	if (!isInited()) {
        ARLOGe("Error: Cannot add trackable. artoolkitX not initialised.\n");
		return false;
	}
	...
	if (!trackable) {
        ARLOGe("WebARKitController::addTrackable(): NULL trackable.\n");
		return false;
	}
...
#if HAVE_NFT
    if (trackable->type == WebARKitTrackable::NFT) {
        if (!doNFTMarkerDetection)
...
bool WebARKitController::removeTrackable(int UID)
{
    WebARKitTrackable *trackable = findTrackable(UID);
    if (!trackable) {
        ARLOGe("WebARKitController::removeTrackable(): could not find trackable (UID=%d).\n");
        return (false);
    }
...
// private
bool WebARKitController::removeTrackable(WebARKitTrackable* trackable)
{
    ARLOGd("WebARKitController::removeTrackable(): called\n");
	if (!trackable) {
        ARLOGe("WebARKitController::removeTrackable(): NULL trackable.\n");
		return false;
	}

	int UID = trackable->UID;
    std::vector<WebARKitTrackable *>::iterator position = std::find(m_trackables.begin(), m_trackables.end(), trackable);
    ...
    if (countTrackables(WebARKitTrackable::SINGLE) + countTrackables(WebARKitTrackable::MULTI) + countTrackables(WebARKitTrackable::MULTI_AUTO) == 0) {
        if (doSquareMarkerDetection)
            ARLOGi("Last square marker removed; disabling square marker tracking.\n");
        doSquareMarkerDetection = false;
    }
#if HAVE_NFT
    if (countTrackables(WebARKitTrackable::NFT) == 0) {
        if (doNFTMarkerDetection)
...
#endif
#if HAVE_2D
    if (countTrackables(WebARKitTrackable::TwoD) == 0) {
        if (doTwoDMarkerDetection)
...
unsigned int WebARKitController::countTrackables() const
{
	return ((unsigned int)m_trackables.size());
}

unsigned int WebARKitController::countTrackables(WebARKitTrackable::TrackableType trackableType) const
{
...
#if HAVE_2D
bool WebARKitController::load2DTrackerImageDatabase(const char* databaseFileName)
{
    removeAllTrackables();
    std::vector<WebARKitTrackable *> newTrackables = m_twoDTracker->loadImageDatabase(std::string(databaseFileName));
...
bool WebARKitController::save2DTrackerImageDatabase(const char* databaseFileName)
{
...
WebARKitTrackable* WebARKitController::getTrackableAtIndex(unsigned int index)
{
    if (index >= m_trackables.size()) return NULL;
    return m_trackables[index];
}

WebARKitTrackable* WebARKitController::findTrackable(int UID)
{

    std::vector<WebARKitTrackable *>::const_iterator it = m_trackables.begin();
...
bool WebARKitController::loadOpticalParams(const char *optical_param_name, const char *optical_param_buff, const long optical_param_buffLen, const ARdouble projectionNearPlane, const ARdouble projectionFarPlane, ARdouble *fovy_p, ARdouble *aspect_p, ARdouble m[16], ARdouble p[16])
{
...
int WebARKitController::webVideoPushInitC(int videoSourceIndex, int width, int height, const char *pixelFormat, int camera_index, int camera_face)
{
    ...
    if (!vs) {
        ...
        if (!vs->isOpen() || !vs->isRunning()) {
            ...
            ARLOGe("WebARKitController::webVideoPushInitC: WebARKitVideoSource is either closed or already running.\n");
        }
    }
    return ret;
}
...
/*jint WebARKitController::androidVideoPushInit(JNIEnv *env, jobject obj, jint videoSourceIndex, jint width, jint height, const char *pixelFormat, jint camera_index, jint camera_face)
{
    if (videoSourceIndex < 0 || videoSourceIndex > (m_videoSourceIsStereo ? 1 : 0)) return -1;
    ...
    if (!vs) {
        ARLOGe("WebARKitController::androidVideoPushInit: no ARVideoSource.\n");
    } else {
        if (!vs->isOpen() || !vs->isRunning()) {
            ret = vs->androidVideoPushInit(env, obj, width, height, pixelFormat, camera_index, camera_face);
        } else {
            ARLOGe("WebARKitController::androidVideoPushInit: ARVideoSource is either closed or already running.\n");
        }
    }
done:
...
jint WebARKitController::androidVideoPush1(JNIEnv *env, jobject obj, jint videoSourceIndex, jbyteArray buf, jint bufSize)
{
    if (videoSourceIndex < 0 || videoSourceIndex > (m_videoSourceIsStereo ? 1 : 0)) return -1;
    ...
    ARVideoSource *vs = (videoSourceIndex == 0 ? m_videoSource0 : m_videoSource1);
    if (!vs) {
        ARLOGe("WebARKitController::androidVideoPush1: no ARVideoSource.\n");
    } else {
        if (vs->isRunning()) {
            ret = vs->androidVideoPush1(env, obj, buf, bufSize);
        } else {
            ARLOGe("WebARKitController::androidVideoPush1: ARVideoSource is not running.\n");
        }
    }
done:
...
jint WebARKitController::androidVideoPush2(JNIEnv *env, jobject obj, jint videoSourceIndex,
                                     ...
                                     jobject buf3, jint buf3PixelStride, jint buf3RowStride)
{
    ...
    ARVideoSource *vs = (videoSourceIndex == 0 ? m_videoSource0 : m_videoSource1);
    if (!vs) {
        ARLOGe("WebARKitController::androidVideoPush2: no ARVideoSource.\n");
    } else {
        if (vs->isRunning()) {
            ...
            ARLOGe("WebARKitController::androidVideoPush2: ARVideoSource is not running.\n");
        }
    }
done:
...
jint WebARKitController::androidVideoPushFinal(JNIEnv *env, jobject obj, jint videoSourceIndex)
{
    if (videoSourceIndex < 0 || videoSourceIndex > (m_videoSourceIsStereo ? 1 : 0)) return -1;
    ...
    ARVideoSource *vs = (videoSourceIndex == 0 ? m_videoSource0 : m_videoSource1);
    if (!vs) {
        ARLOGe("WebARKitController::androidVideoPushFinal: no ARVideoSource.\n");
    } else {
        if (vs->isOpen()) {
            ret = vs->androidVideoPushFinal(env, obj);
        } else {
            ARLOGe("WebARKitController::androidVideoPushFinal: ARVideoSource is not open.\n");
        }
    }
done:
