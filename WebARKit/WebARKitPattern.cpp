...
WebARKitPattern::WebARKitPattern() :
	m_width(0.0f),
    m_height(0.0f),
    m_imageSizeX(0),
    m_imageSizeY(0),
...
bool WebARKitPattern::loadTemplate(int patternID, const ARPattHandle *arPattHandle, float width)
{
...
bool WebARKitPattern::loadMatrix(int barcodeID, AR_MATRIX_CODE_TYPE type, float width)
{
...
#if HAVE_NFT
bool WebARKitPattern::loadISet(const AR2ImageSetT *imageSet, float nftScale)
{
...
bool WebARKitPattern::load2DTrackerImage(std::shared_ptr<unsigned char> arImage, float imageX, float imageY, float width, float height)
{
...
void WebARKitPattern::freeImage()
{
	if (m_image) {
		delete[] m_image;
		m_image = NULL;
