...
void OrbFeatureDetector::SetFeatureDetector(int detectorType)
{
    if (detectorType == 0) {
...
void OrbFeatureDetector::CreateORBFeatureDetector()
{
    _featureDetector = cv::ORB::create();
    _matcher = cv::BFMatcher::create();
}

bool OrbFeatureDetector::AddDescriptorsToDictionary(int id, cv::Mat descriptors)
{
...
std::vector<cv::KeyPoint> OrbFeatureDetector::DetectAndCompute(cv::Mat frame, cv::Mat mask, cv::Mat &desc) {
  std::vector<cv::KeyPoint> kp;
  _featureDetector->detectAndCompute(frame, mask, kp, desc);
  return kp;
}

std::vector< std::vector<cv::DMatch> >  OrbFeatureDetector::MatchFeatures(cv::Mat first_desc, cv::Mat desc)
{
